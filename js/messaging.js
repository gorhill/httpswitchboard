...
function defaultHandler(request, sender, callback) {
    ...
    switch ( request.what ) {
        case 'loadUbiquitousAllowRules':
            return HTTPSB.loadUbiquitousWhitelists();
    ...
    switch ( request.what ) {
        ...
        case 'getUserSettings':
            response = HTTPSB.userSettings;
            break;

        case 'gotoExtensionURL':
            HTTPSB.utils.gotoExtensionURL(request.url);
            break;

        case 'gotoURL':
            HTTPSB.utils.gotoURL(request);
            break;
        ...
        default:
            // console.error('HTTPSB> messaging.js / defaultHandler > unknown request: %o', request);
            break;
    }
...
var onConnect = function(port) {
    ...
    if ( !port.name ) {
        throw 'HTTPSB> messaging.js / onConnectHandler(): no port name!';
    }

    // Port should not already exist.
    if ( nameToPortMap[port.name] ) {
        throw 'HTTPSB> messaging.js / onConnectHandler():  port already exists!';
    }
